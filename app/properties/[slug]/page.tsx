...
import PreRegisterButton from "@/app/components/PreRegisterButton"; // NEW BUTTON IMPORT
...
export default async function DynamicPropertyPage(
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen relative pb-20 pt-[80px]">
        <style>{`footer, #global-footer { display: none !important; }`}</style>
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <div className="hidden md:flex items-center gap-3 border-r border-gray-300 pr-6">
              {property.partner_logo ? (
                <img src={property.partner_logo} alt="Partner Logo" className="h-10 w-auto object-contain max-w-[100px]" />
              ) : (
                <div className="w-12 h-8 bg-gray-100 flex items-center justify-center font-bold text-[10px] text-gray-500 rounded">LOGO</div>
              )}
            </div>
            <div>
              <h1 className="text-[22px] md:text-[26px] font-bold text-[#0F1A2A] leading-none mb-1">{property.title}</h1>
              <p className="text-[#5b6472] text-[13px] md:text-[14px]">{property.location}, {property.city}.</p>
            </div>
          </div>
          <button className="p-2 border border-gray-200 rounded-md text-gray-600 hover:bg-gray-50 transition-colors"><Share2 size={18} /></button>
        </div>
      </div>
      ...
      <div className="sticky top-[80px] z-40 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-[1300px] mx-auto px-6 flex overflow-x-auto scrollbar-hide">
          {['Overview', 'Price', 'Site & Floor Plan', 'Amenities', 'Location'].map((tab, idx) => (
            <a key={idx} href={`#${tab.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`} className={`whitespace-nowrap py-4 px-4 md:px-8 text-[15px] font-medium transition-colors ${idx === 0 ? "text-[#2a2a2a] border-b-[3px] border-[#2a2a2a]" : "text-[#5b6472] hover:text-[#0F1A2A]"}`}>{tab}</a>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8 flex flex-col gap-12">
            ...
            <section id="overview" className="scroll-mt-[160px]">
              ...
              <div className="text-[#374151] text-[15px] md:text-[16px] leading-[1.8]">
                ...
                <div className="prose prose-sm max-w-none text-[#374151]" dangerouslySetInnerHTML={{ __html: property.overview || '' }} />
              </div>
            </section>
            ...
            <section id="price" className="scroll-mt-[160px]">
              <div className="flex justify-between items-end mb-6">
                ...
                <button className="text-[#2a2a2a] border border-[#2a2a2a] px-4 py-1.5 rounded-[6px] text-[14px] font-medium hover:bg-[#f4f7fb] transition-colors hidden md:block">Complete Costing Details</button>
              </div>
              <div className="border border-gray-200 rounded-[12px] overflow-hidden shadow-sm">
                <div className="grid grid-cols-4 bg-[#393939] text-white px-5 py-4 text-[14px] md:text-[15px] font-medium"><div>Type</div><div>Sqft.</div><div>Price</div><div>Price Breakup</div></div>
                {configurations?.map((config, idx) => (
                  <div key={config.id} className={`grid grid-cols-4 px-5 py-4 text-[14px] md:text-[15px] text-[#374151] items-center ${idx !== configurations.length - 1 ? 'border-b border-gray-200' : ''}`}>
                    <div className="font-medium">{config.unit_type}</div><div>{config.area_label}</div><div>{config.price || "₹On Request*"}</div>
                    <div><button className="text-[#2a2a2a] font-medium hover:underline text-[13px] md:text-[14px]">View Breakup</button></div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section id="site-floor-plan" className="scroll-mt-[160px]">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="relative rounded-[16px] overflow-hidden border border-gray-200 shadow-sm group cursor-pointer h-[240px]">
                  ...
                  <div className="absolute bottom-0 left-0 w-full bg-[#2a2a2a] text-white text-center py-3 font-medium text-[15px]">{property.title} - Master Plan</div>
                </div>
                <div className="relative rounded-[16px] overflow-hidden border border-gray-200 shadow-sm group cursor-pointer h-[240px]">
                  <img src="/floorplan.jpeg" alt="Floor Plan" className="w-full h-full object-cover blur-[6px] group-hover:blur-[4px] transition-all" />
                  <div className="absolute inset-0 bg-black/10" />
                  <div className="absolute bottom-0 left-0 w-full bg-[#2a2a2a] text-white text-center py-3 font-medium text-[15px]">{property.title} - Floor Plan</div>
                </div>
              </div>
            </section>
            ...
            <section id="amenities" className="scroll-mt-[160px]">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-8 gap-x-4">
                {amenities?.map((item: any, i) => (
                  <div key={i} className="flex items-center gap-4 text-[#374151] text-[15px] md:text-[16px] font-medium"><div className="w-8 flex justify-center">{getAmenityIcon(item.amenities.name)}</div><span>{item.amenities.name}</span></div>
                ))}
              </div>
            </section>
            ...
            <section id="location" className="scroll-mt-[160px]">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {locationAdvantages?.map((loc) => (
                  <div key={loc.id} className="bg-[#d9d7d7] rounded-[12px] p-3 flex items-center gap-4 border border-blue-50/50 hover:shadow-sm transition-shadow">
                    <div className="bg-white rounded-[8px] border border-gray-100 shadow-sm min-w-[64px] h-[64px] flex flex-col items-center justify-center shrink-0"><span className="text-[18px] font-bold text-[#2a2a2a] leading-none mb-1">{loc.distance_value}</span><span className="text-[12px] text-gray-500 font-medium leading-none">{loc.distance_unit}</span></div>
          ...
          <div className="lg:col-span-4 relative">
            <div className="sticky top-[140px] flex flex-col gap-6">
              ...
              <div className="bg-[#d9d7d7] rounded-[16px] p-6 border border-blue-50">
                <div className="grid grid-cols-2 gap-x-4 gap-y-6 mb-8">
                  <div className="flex flex-col gap-1"><div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#2a2a2a] mb-1 shadow-sm"><Building2 size={16} /></div><span className="text-[11px] text-gray-500 font-medium uppercase tracking-wider">Units</span><span className="text-[13px] font-semibold text-[#0F1A2A] leading-snug">{property.units || "N/A"}</span></div>
                  <div className="flex flex-col gap-1"><div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#2a2a2a] mb-1 shadow-sm"><Layers size={16} /></div><span className="text-[11px] text-gray-500 font-medium uppercase tracking-wider">Total floors</span><span className="text-[13px] font-semibold text-[#0F1A2A] leading-snug">{property.total_floors || "N/A"}</span></div>
                  <div className="flex flex-col gap-1"><div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#2a2a2a] mb-1 shadow-sm"><Scaling size={16} /></div><span className="text-[11px] text-gray-500 font-medium uppercase tracking-wider">Built-up Area</span><span className="text-[13px] font-semibold text-[#0F1A2A] leading-snug">{property.built_up_area || "N/A"}</span></div>
                  <div className="flex flex-col gap-1"><div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#2a2a2a] mb-1 shadow-sm"><Map size={16} /></div><span className="text-[11px] text-gray-500 font-medium uppercase tracking-wider">Land Parcel</span><span className="text-[13px] font-semibold text-[#0F1A2A] leading-snug">{property.land_parcel || "N/A"}</span></div>
                  <div className="flex flex-col gap-1"><div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#2a2a2a] mb-1 shadow-sm"><Clock size={16} /></div><span className="text-[11px] text-gray-500 font-medium uppercase tracking-wider">Property Status</span><span className="text-[13px] font-semibold text-[#0F1A2A] leading-snug">{property.property_status || "N/A"}</span></div>
                  <div className="flex flex-col gap-1"><div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#2a2a2a] mb-1 shadow-sm"><CalendarDays size={16} /></div><span className="text-[11px] text-gray-500 font-medium uppercase tracking-wider">Possession Date</span><span className="text-[13px] font-semibold text-[#0F1A2A] leading-snug">{property.possession_date || "N/A"}</span></div>
                </div>

                <div className="mb-6">
                  <span className="text-[22px] md:text-[26px] font-bold text-[#0F1A2A] leading-none">{property.starting_price || "Price on Request"}</span>
                  <span className="text-[16px] font-semibold text-[#0F1A2A] ml-2">Onwards</span>
                </div>

                <div className="flex flex-col gap-3">
                  <button className="w-full py-3 rounded-[8px] border-2 border-[#2a2a2a] text-[#2a2a2a] font-semibold text-[15px] hover:bg-[#2a2a2a] hover:text-white transition-all flex items-center justify-center gap-2">Download Brochure</button>
              ...
              <div className="bg-white rounded-[16px] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-gray-100">
                ...
                <form className="flex flex-col gap-5">
                  ...
                  <label className="flex items-start gap-3 mt-2 cursor-pointer">
                    ...
                    <span className="text-[11px] text-gray-500 leading-snug">I authorize {property.title}...</span>
                  </label>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
        <div className="bg-white shadow-[0_8px_30px_rgba(0,0,0,0.15)] border border-gray-100 rounded-full px-6 py-3 flex items-center gap-4 cursor-pointer hover:scale-105 transition-transform">
          <div className="w-10 h-10 rounded-full bg-[#f4f7fb] text-[#2a2a2a] flex items-center justify-center"><Phone size={20} /></div>
          <div className="flex flex-col pr-2"><span className="text-[12px] text-gray-500 font-medium leading-none mb-1">Instant Callback</span><span className="text-[18px] font-bold text-[#0F1A2A] leading-none">+91 8071737887</span></div>
        </div>
      </div>
